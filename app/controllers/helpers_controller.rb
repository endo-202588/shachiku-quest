class HelpersController < ApplicationController
  def helping
    return redirect_to help_requests_path, alert: 'ヘルパーではありません' unless current_user&.helper?

    @help_request =
      HelpRequest.includes(task: :user).find_by(helper_id: current_user.id, status: :matched)&.decorate
